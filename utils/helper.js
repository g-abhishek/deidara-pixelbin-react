...
nunjucksEnv.addFilter("postmanified", function (swagger, callback) {
  Converter = require('openapi-to-postmanv2'),
  Converter.convert({ type: 'json', data: JSON.parse(JSON.stringify(swagger)) }, {folderStrategy: 'Tags'}, (err, conversionResult) => {
  ...
  if (schema.type) {
    ...
    if (schema.type == "object") {
      if (schema.additionalProperties) {
         return `Joi.object().pattern( /\w\d/,${getJSDataTypeForJoi(schema.additionalProperties)})`
      }
    }
    return {
    ...
    if (schema["$ref"]) {
        ...
        if (ref)
            return `${prepend ? prepend : 'this'}.${ref.substring(
        ref.lastIndexOf("/") + 1
      )}()`;
    }
...
function getSchemaLinkNotationForJS(schema) {
    ...
    if (schema["$ref"]) {
        const ref = schema["$ref"];
        if (ref) return '['+ref.substring(ref.lastIndexOf("/") + 1) + '](#' +ref.substring(ref.lastIndexOf("/") + 1)+')';
    }
    ...
    if (schema.type) {
        ...
        if (schema.type == "object") {
          if (schema.additionalProperties && typeof(schema.additionalProperties) == 'object') {
            if (schema.additionalProperties.type && schema.additionalProperties.type == 'object') {
              return "string"
            }
    ...
    if (schema.type) {
        ...
        if (schema.type == "object") {
          if (schema.additionalProperties && typeof(schema.additionalProperties) == 'object') {
            ...
            return "[String: " + getSchemaLinkNotationForSwift(schema.additionalProperties) + "]";
          } else {
            return "[String: Any]"
          }
        }
...
function getSwiftDataTypeFromResponseType(responseType) {
    if (responseType == 'application/pdf') {
        return 'Data'
    } else if (responseType == 'application/csv' || responseType == 'text/csv' || responseType == 'application/octet-stream') {
        return 'Data'
    } else {
        return '[String: Any]';
    }
...
function getKotlinDataTypeFromSchema(schema) {
  ...
  if (schema.type) {
    ...
    if(schema.type == "object"){
      if(schema.additionalProperties){
        return "HashMap<String," + getKotlinDataTypeFromSchema(schema.additionalProperties) + ">";
      }else{
...
function getSchemaLinkNotationForKotlin(schema) {
  ...
  if (schema.type) {
    ...
    if(schema.type == "object"){
      if(schema.additionalProperties){
        return "HashMap<String," + getSchemaLinkNotationForKotlin(schema.additionalProperties) + ">";
      }else{
        return "HashMap<String,Any>";
      }
    }
...
function getSchemaLinkNotationForJava(schema) {
  ...
  if (schema.type) {
    ...
    if(schema.type == "object"){
      if(schema.additionalProperties){
        return "HashMap<String," + getSchemaLinkNotationForJava(schema.additionalProperties) + ">";
      }else{
...
nunjucksEnv.addFilter("getApplicationModelClass", function (schema) {
  ...
  var datatypesArray = ['String','Object','Integer','Double','Boolean','List<String>','List<Double>','List<Integer>','HashMap<String,Object>'];
...
function getJavaDataTypeFromSchema(schema, model) {
  ...
  if (schema.type) {
    ...
    if(schema.type == "object"){
      if(schema.additionalProperties){
        return "HashMap<String," + getJavaDataTypeFromSchema(schema.additionalProperties) + ">";
      }else{
        return "Object";
      }
    }
...
function getFilterParamsForPlatformApplicationPaginator(parameters) {
    return parameters.filter(
        (p) => (p.name != "company_id") & (p.name != "application_id")
    );
}

nunjucksEnv.addFilter(
    "filterParamsForPlatformPaginator",
    getFilterParamsForPlatformPaginator
);
